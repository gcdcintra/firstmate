...
make_fake_root() {
  ...
  # fm-gh-lib.sh: teardown sources it so its landed-work GitHub lookups name a
  # repository instead of letting the CLI infer one.
  ln -s "$ROOT/bin/fm-gh-lib.sh" "$fake/bin/fm-gh-lib.sh"
