...
| `fm-session-lock-lib.sh` | Shared session-lock harness identity (ancestry walk, holder liveness, live-foreign-owner test, fork-source evidence) for fm-lock.sh, the Claude Stop auto-arm, and the Claude turn-end guard |
...
| `fm-nm-custody.sh`       | Classify a branch's no-mistakes custody, and step around one stranded by a dead run  |
...
| `fm-quota-axi-lib.sh`    | Shared `quota-axi` compatibility floor, and the one bounding ladder every `quota-axi` call runs under |
| `fm-quota-kill-lib.sh`   | Single owner of the vendor usage-limit recognizer that separates a quota kill from a genuine agent failure |
| `fm-quota-advice.sh`     | Report every quota window and the one that binds before an expensive validation run; advisory only, never withholds a spawn |
...
| `fm-delegation-lib.sh`   | Single owner of what makes a tool name delegation-shaped, and of a crewmate's open-delegation record - the signature of a worker blocked behind a helper of its own |
| `fm-delegation-event.sh` | The only writer of that record; a crewmate's tool-call hooks open, close, and clear it |
...
| `fm-branch-poll.sh`      | Sweep every clone's default branch and wake once per commit that settles red, with refusal-vs-failure evidence inline |
| `fm-branch-watch-lib.sh` | Own the per-project default-branch verdict record, its validation, and its surfaced-wake boundary |
| `fm-check-register.sh`   | Bind an intentional custom watcher check to its current bytes                       |
| `fm-check-lib.sh`        | Validate custom-check registrations and prepare private execution snapshots          |
| `fm-gh-lib.sh`           | Resolve and pin the repository for every `gh`/`gh-axi` pull-request and issue query, refusing when none can be established |
| `fm-gh-repo-guard.sh`    | Refuse any `bin/` GitHub pull-request or issue query that does not name its repository |
...
| `fm-worktree-owner-lib.sh` | Shared proof that a task's recorded worktree still belongs to that task before anything acts on it destructively |
| `fm-worktree-owner.sh`   | Inspect a task's worktree-ownership record, or restore one that was lost while the worktree is still the task's |
