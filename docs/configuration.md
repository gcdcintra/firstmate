...
FM_CPU_PROGRESS_MIN_TICKS_PER_MIN=120   # minimum CPU ticks per minute (utime+stime+cutime+cstime from /proc/<pid>/stat) the worker process must accumulate for a wedge escalation to be deferred; only that `progressing` verdict defers, and a flat or unmeasurable counter escalates unchanged
