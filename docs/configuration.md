...
FM_STALE_ESCALATE_SECS=240         # seconds without pane output before a provably-working stale pane escalates; stale panes whose crew is not provably working surface immediately unless they declare the pause verb
...
FM_WEDGE_DECLARED_WAIT_MAX_DEFER_SECS=3600   # per-pane cap, against that same episode, on how long a declared wait may hold a busy-turn pane on the FM_PAUSE_RESURFACE_SECS recheck cadence instead of the wedge cadence; past it the declaration no longer lengthens the cadence and the pane escalates as a possible wedge
