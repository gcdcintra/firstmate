...
## Default-branch watch (config/branch-watch / state/branch-watch)

Firstmate follows a task's own pull request until it is green and then stops, so nothing looked at the branch that work merges onto once it landed.
...
A failure is only forgotten on sustained recovery, never on a pass that simply did not get to that clone, so a transient blip cannot quietly excuse the permanent failure that arrives later.
...
FM_CREW_STATE_PATCH_SCAN=200  # commits of local-vs-pipeline divergence fm-crew-state.sh will diff to prove a run is validating this checkout's code; past it the run is unattributed, so a crew whose branch base is that far behind the default branch reads from its pane and status log instead of its run step (the reader names the ceiling in its detail when it declines for this reason)
FM_CREW_STATE_BIN=bin/fm-crew-state.sh   # test override for the current-state reader used by working/paused watcher triage and by the wedge path's pipeline-activity read
...
FM_STALE_ESCALATE_SECS=240         # seconds without pane output before a provably-working stale pane escalates, unless a still-standing declared wait or another tier of the wedge evidence hierarchy defers it; stale panes whose crew is not provably working surface immediately unless they declare the pause verb
...
FM_ENDPOINT_ABSENCE_CONFIRM_POLLS=2   # consecutive polls an endpoint must repeat the same confidently-absent backend verdict before it wakes as gone, which absorbs a teardown or spawn race without waiting out the wedge timer
FM_WEDGE_DEMAND_INSPECT_COUNT=3    # consecutive wedge-path wakes on the same unchanged pane - escalations and declared-wait rechecks alike - before demand-deep-inspection is added
FM_CPU_PROGRESS_MIN_TICKS_PER_MIN=120   # minimum CPU ticks per minute (utime+stime+cutime+cstime from /proc/<pid>/stat) the worker process must accumulate for a wedge escalation to be deferred; deferral is offered only while a turn is still in progress on that pane, so a worker idle at its prompt escalates on the ordinary cadence whatever it reads; only that `progressing` verdict defers, and a flat or unmeasurable counter escalates unchanged
FM_CPU_PROGRESS_WINDOW=45          # shortest worker-CPU sampling window trusted for a fresh verdict; a shorter one is scheduler noise, so the pane's previous verdict is carried forward until the window matures and a pane with no verdict yet reads unknown, which escalates
FM_CPU_PROGRESS_WINDOW_MAX=300     # widest worker-CPU sampling window trusted for a verdict; past it the anchor is reset and the verdict is unknown, which escalates
FM_CPU_PROGRESS_MAX_DEFER_SECS=7200   # per-pane cap on how long measured CPU progress may defer one pane's wedge escalation, read against the pane's one shared deferral episode; spent once, after which the pane escalates on the FM_STALE_ESCALATE_SECS cadence until it goes genuinely active again
FM_WEDGE_PIPELINE_MAX_DEFER_SECS=3600   # per-pane cap, against that same episode, on how long an attributed pipeline run's own advancing activity clock may defer a wedge escalation; deliberately shorter than the CPU cap because it is evidence about the pipeline's process rather than the worker's, so it says the task is progressing and cannot say the worker is
FM_WEDGE_DECLARED_WAIT_MAX_DEFER_SECS=3600   # per-pane cap, against that same episode, on how long a declared wait may hold a pane on the FM_PAUSE_RESURFACE_SECS recheck cadence instead of the wedge cadence; applies on every wedge path the pane can reach while its status line still declares the wait, including a pane whose crew the current-state reader reports authoritatively working under that same line; past the cap the declaration no longer lengthens the cadence and the pane escalates as a possible wedge
FM_WEDGE_PIPELINE_SAMPLE_SECS=240   # defaults to FM_STALE_ESCALATE_SECS; shortest interval between two pipeline-activity reads for one pane, so an aging pane costs at most one bounded no-mistakes call per interval rather than one per poll
FM_WEDGE_DELEGATION_BLOCK_SECS=900   # how long one delegation-shaped tool call may stay open before the worker counts as blocked behind a helper of its own; past it the pipeline and CPU tiers are denied outright rather than capped, because both are then measuring a different process, and the escalation names the open call and demands deep inspection at once; a shorter call buys and costs nothing, and an unreadable or absent record never triggers it
FM_WEDGE_DECLARED_WAIT_NOTE_MAX=160   # longest declared-wait status line quoted back inside a wedge escalation or recheck reason
