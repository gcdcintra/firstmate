...
It reports `attached` only after the target keeps passing the health gate for a bounded verification window, stays live across identity-matched successors, and takes a cycle over itself rather than returning while nothing holds the singleton, so repeated arming converges on exactly one live cycle.
Every `FAILED` result names what failed and quotes a bounded tail of the watcher's own stderr; [`watcher-continuity.md`](watcher-continuity.md) owns the full arm-layer cycle contract.
