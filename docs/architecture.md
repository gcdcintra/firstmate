...
Measured CPU progress defers the escalation up to `FM_CPU_PROGRESS_MAX_DEFER_SECS`, and only a `progressing` verdict defers: a flat counter and every unmeasurable case escalate exactly as before, and each escalation carries the CPU evidence it observed.
