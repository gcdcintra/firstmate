...
A killed endpoint is a separate wake kind (`gone`) rather than a wedge, because no pane-shaped heuristic can tell the two apart: a dead endpoint and a hung agent hold the same frozen frame and read the same flat CPU counter, so the CPU-aware wedge detection above does not help.
