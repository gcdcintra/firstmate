...
A declared `paused:` or `captain-held` line covers the worker's claim and moves the pane onto the `FM_PAUSE_RESURFACE_SECS` recheck cadence instead of the wedge cadence, which is what a worker holding a turn open past its bound previously never got.
