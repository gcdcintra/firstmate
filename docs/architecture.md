...
Every wedge escalation, from either path, first consults the worker process's own CPU counter through [`bin/fm-cpu-progress-lib.sh`](../bin/fm-cpu-progress-lib.sh), because a worker only reaches its status file between turns and therefore cannot declare a pause from inside one long tool-driven turn.
Deferral is offered only while a turn is still in progress on that pane, i.e. busy with no completed turn, so a worker idle at its prompt escalates on the ordinary cadence whatever it reads; the three non-busy wedge paths, the provably-working stale among them, are never held back by a CPU reading.
On that busy-turn path measured CPU progress defers the escalation up to `FM_CPU_PROGRESS_MAX_DEFER_SECS`, and only a `progressing` verdict defers: a flat counter and every unmeasurable case escalate exactly as before, and each escalation from either path carries the CPU evidence it observed.
Because deferral is gated on an exact busy verdict, a harness that never produces one - Codex (`unknown codex-unverified`) and standalone Kimi (`unknown kimi-unverified`) - is never deferred even inside one long turn, so the false-alarm loop is unchanged for those harnesses; that gap belongs to the busy-state contract in [`bin/fm-busy-lib.sh`](../bin/fm-busy-lib.sh), and [`bin/fm-cpu-progress-lib.sh`](../bin/fm-cpu-progress-lib.sh)'s limits list states it in full.
That allowance is a per-pane budget spent once, not a window that re-opens after each escalation, so a wedge that keeps burning CPU escalates on the ordinary `FM_STALE_ESCALATE_SECS` cadence past it - saying the budget is spent and what the CPU still reads - until the pane goes genuinely active again.
...
Resolving the worker's own pid for the wedge path's CPU-progress check is a third separate read, implemented for tmux and Herdr only, with `fm_backend_agent_pid`'s comment in `bin/fm-backend.sh` as the single owner of its contract; zellij, orca, and cmux expose no verified equivalent, so a wedge escalation on those backends is never CPU-deferred.
