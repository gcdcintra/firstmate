...
Actionable wakes include captain-relevant status signals, no-verb signals whose crew is not provably working, authenticated check output such as PR merge polling, an X-mode mention, or a cloned project's default branch settling red ([`bin/fm-branch-poll.sh`](../bin/fm-branch-poll.sh)), stale panes whose crew is not provably working whether their status log looks terminal or non-terminal, provably-working stale panes that persist past `FM_STALE_ESCALATE_SECS`, declared external waits that remain paused past `FM_PAUSE_RESURFACE_SECS`, endpoints confirmed killed or agent-less, and heartbeat backstop hits.
...
A killed endpoint is a separate wake kind (`gone`) rather than a wedge, because no pane-shaped heuristic can tell the two apart: a dead endpoint and a hung agent hold the same frozen frame and read the same flat CPU counter, so no tier of the wedge evidence hierarchy below can separate them.
The watcher instead asks `fm_backend_agent_state` ([`bin/fm-backend.sh`](../bin/fm-backend.sh)), and wakes only on its two confident non-agent verdicts: `missing` (the endpoint is authoritatively absent, which for tmux includes a dead server or session) and `dead` (the endpoint survives running a bare shell where an agent belongs).
A `dead` report also names the husk shell's working directory when it has drifted off the task worktree, but only on a backend with a passive current-path read, which is tmux today; `fm_backend_current_path`'s comment in [`bin/fm-backend.sh`](../bin/fm-backend.sh) owns that coverage and why an active pwd probe is wrong here.
`alive`, `ambiguous`, `unreadable`, and `unverified` never wake and never divert a window, so a genuinely wedged worker with a live pane escalates exactly as it did and a backend with no recovery classifier is unchanged.
A verdict must repeat for `FM_ENDPOINT_ABSENCE_CONFIRM_POLLS` consecutive polls before it wakes, which absorbs a teardown mid-flight or a spawn whose endpoint is recorded just before its agent starts, and it wakes once per absence episode because an absent endpoint never returns on its own.
An absence is logged rather than woken on when the worker already reported a terminal outcome, or when a declared pause or captain hold already owns the exited-agent case on its own bounded recheck cadence; the triage-log line names which of the two applied.
Neither absorbed case is claimed - the window stays on the path it would have taken, because claiming it would cancel the very `FM_PAUSE_RESURFACE_SECS` recheck that makes a long wait safe to absorb - and neither marks the episode delivered, so a status that later moves off the accounted-for verb can still wake `gone`.
A busy pane is otherwise exempt from staleness, but only until its latest `state/<id>.turn-ended` marker reaches `FM_BUSY_TURN_MAX_SECS`, or its `state/<id>.meta` spawn record reaches that age before any turn completes; past that bound it is routed through the same wedge escalation, with the same escalation count and `demand-deep-inspection` marker, for inspection only - never an automatic interrupt, signal, or restart.
Every wedge escalation, from either path, first goes through the ordered evidence hierarchy owned by [`bin/fm-wedge-evidence-lib.sh`](../bin/fm-wedge-evidence-lib.sh), because the four signals a supervisor would consult do not cover the same subject and the order is what makes them comparable.
The harness's own busy verdict covers the worker's turn and is already the gate above; the escalation names it and its source rather than silently overriding it.
A declared `paused:` or `captain-held` line covers the worker's claim and moves the pane onto the `FM_PAUSE_RESURFACE_SECS` recheck cadence instead of the wedge cadence, on every wedge path a pane can reach while that line still stands.
Whether the declaration is current is decided inside the hierarchy, from the last status line on each call, so a crew that has moved off it earns nothing on the next poll.
That covers the pane whose crew [`bin/fm-crew-state.sh`](../bin/fm-crew-state.sh)'s run-step precedence reports authoritatively working under its own declaration, which a crewmate that backgrounds its blocking pipeline call produces: run-step precedence correctly moves that pane off the pause cadence marker and onto the wedge timer, and reading the declaration on that timer too is what stops one declaration being answered differently depending on which path its pane took.
The attributed pipeline run's own activity clock covers the pipeline's agent, read through [`bin/fm-crew-state.sh`](../bin/fm-crew-state.sh)'s `--pipeline-activity` mode so it reuses that file's branch and code-identity attribution; a sibling crew's live run therefore can never be credited to a quiet pane, an actively advancing run defers, and a run parked at a gate deliberately earns nothing because it is waiting for this worker.
The worker process's own CPU counter ([`bin/fm-cpu-progress-lib.sh`](../bin/fm-cpu-progress-lib.sh)) remains the last resort, still offered only while a turn is in progress on that pane, because a worker idle at its prompt reads 0.58-3.82 ticks/s against a 2.0 floor.
That ordering exists because the CPU counter alone is measured on a process that is correct to be near-idle for most of a ship task's life: during a validation run the worker is blocked in one `axi run` call while the work happens in the pipeline's separate agent.
One condition denies the pipeline and CPU tiers outright rather than capping them: an open delegation-shaped tool call older than `FM_WEDGE_DELEGATION_BLOCK_SECS`, recorded by the crewmate's own tool-call hooks under the contract in [`bin/fm-delegation-lib.sh`](../bin/fm-delegation-lib.sh).
A worker blocked behind a helper of its own is the one shape where those two tiers are readings of a different process entirely, so the escalation leads with the open call, still carries the other readings behind it, and demands deep inspection on its first alarm rather than its third.
That is an addition to the escalation-count rule, never a replacement: the count is what forces a real look when no single reading explains a pane, and it is what caught this shape before the record existed.
Every escalation carries the whole ordered reading, so an alarm a supervisor must dismiss can be dismissed by reading it rather than by spending a turn re-deriving the same four reads.
Two rules keep this from becoming a quieter watcher that stops looking, which would be a worse defect than the false alarms it removes.
The escalation ladder keeps counting through every deferral and every declared-wait recheck, so `FM_WEDGE_DEMAND_INSPECT_COUNT` stays reachable for a worker whose declared wait is inaccurate or whose pipeline advances without it - the shape that blocked a worker on a helper process for over two hours while every one of those signals read healthy.
And all tiers share ONE deferral episode per pane with per-tier caps read against that single epoch, so a spent episode disables every soft tier at once and tiers cannot be chained into a longer silence than the longest single cap.
Below the deep-inspection threshold a declared-wait recheck deliberately omits the possible-wedge segment so away mode reads it as the pause recheck it is; at the threshold it switches to the wedge form, because that segment is the only path a worker wedged behind a busy-looking pane reaches an away captain.
Because deferral by the CPU tier is gated on an exact busy verdict, a harness that never produces one - Codex (`unknown codex-unverified`) and standalone Kimi (`unknown kimi-unverified`) - reaches the declared-wait and pipeline tiers but never that one; that gap belongs to the busy-state contract in [`bin/fm-busy-lib.sh`](../bin/fm-busy-lib.sh), and [`bin/fm-cpu-progress-lib.sh`](../bin/fm-cpu-progress-lib.sh)'s limits list states it in full.
Every unmeasurable case - a missing CLI, a timed-out read, coarse attribution only, an unreadable table, a corrupt episode record - earns nothing and escalates exactly as it did before the hierarchy existed, so the design degrades toward noise and never toward blindness.
Past a tier's cap the pane escalates on the ordinary `FM_STALE_ESCALATE_SECS` cadence, saying which evidence stopped holding it back, until the pane goes genuinely active again.
[`bin/fm-wedge-evidence-lib.sh`](../bin/fm-wedge-evidence-lib.sh)'s limits list owns the full statement of the wedge shapes none of the tiers can see.
In away mode the watcher runs as the daemon's child with both wedge paths unchanged - the busy-turn bound is not gated on away mode - and the daemon classifies every reason the watcher prints.
The daemon force-escalates any stale reason carrying the shared possible-wedge segment `FM_CLASSIFY_WEDGE_REASON_SEGMENT`, whose one definition in [`bin/fm-classify-lib.sh`](../bin/fm-classify-lib.sh) both the watcher's reason composer and the daemon's matcher read, so a rewording on either side cannot silently sever them.
That forced escalation is what surfaces a worker wedged mid-turn to an away captain, because the daemon's own housekeeping has no turn-duration bound and drops a stale marker outright while the pane still looks busy.
Those actionable wakes are written to a durable local queue (`state/.wake-queue`) before detector state advances, so a missed process exit can be recovered by draining the queue.
Queuing a wake is not delivering it: each harness has its own recognizer for which watcher stdout lines are wakes rather than status lines, and all of them - [`bin/fm-watch-arm.sh`](../bin/fm-watch-arm.sh), [`bin/fm-claude-stop-autoarm.sh`](../bin/fm-claude-stop-autoarm.sh), [`bin/fm-watch-checkpoint.sh`](../bin/fm-watch-checkpoint.sh), and the away-mode daemon's `is_wake_reason` - read the one shared `FM_WAKE_REASON_PREFIX_RE` in [`bin/fm-classify-lib.sh`](../bin/fm-classify-lib.sh), so a wake kind added to the queue vocabulary cannot half-land as queueable but undeliverable.
The two in-process plugins - [`.pi/extensions/fm-primary-pi-watch.ts`](../.pi/extensions/fm-primary-pi-watch.ts) and [`.opencode/plugins/fm-primary-watch-arm.js`](../.opencode/plugins/fm-primary-watch-arm.js) - hold the only copies that cannot source bash, and both miss in the same direction: an unrecognized reason is classified as a watcher failure, so a dead worker would surface as a broken watcher.
`tests/fm-pi-watch-extension.test.sh` therefore drives every kind through each one's real close classifier instead of trusting the copy.
...
A pull-based guard (`bin/fm-guard.sh`) warns through supervision tool output if the primary checkout is tangled, if work, process-event sources, or X-mode relay polling needs supervision and the watcher beacon has aged past the grace window, or if queued wakes are waiting to be drained.
It stays silent through the ordinary gap between one watcher cycle exiting and the next arming, and names the exact condition that failed when it does alarm; its header owns why that gate differs from the turn-end guard's.
...
The watcher and daemon share `bin/fm-classify-lib.sh` for captain-relevant status verbs, declared-external-wait vocabulary, the possible-wedge escalation reason segment, and status-scan primitives.
...
The tmux submit core (shared `fm_tmux_submit_enter_core`) treats a busy pane + retries-exhausted + composer-still-pending as a queued Enter (opencode 1.18.4 accepts Enter mid-turn and queues it for after the turn), reported as `empty` so the daemon and `fm-send` do not re-send; an idle pane keeps the `pending` verdict as a genuine swallow. This composer-proof form of the conversion is tmux-specific; [`herdr-backend.md`](herdr-backend.md) owns Herdr's own native-state form of the same busy-queue case and the residual limit that still stands there.
...
It owns task endpoint creation, bounded capture, text/key sends, current-path reads for spawn-time worktree discovery when the backend does not create the worktree itself and for the killed-endpoint husk report, live-window fallback lookup, agent-process liveness probes and worker-process pid resolution where verified, and endpoint teardown.
...
Resolving the worker's own pid for the wedge path's CPU-progress check is a third separate read, implemented for tmux and Herdr only, with `fm_backend_agent_pid`'s comment in `bin/fm-backend.sh` as the single owner of its contract; zellij, orca, and cmux expose no verified equivalent, so a wedge escalation on those backends is never CPU-deferred, though the declared-wait and pipeline-activity tiers above it need no pid and still apply there.
...
Independently, `fm-spawn.sh`, `fm-send.sh`, `fm-teardown.sh`, and `fm-worktree-owner.sh` (its record-writing `claim`, not its read-only `show`) source `bin/fm-gate-refuse-lib.sh` and exit with status 3 before fleet mutation when the gate environment marker is present or the current checkout matches the default no-mistakes gate-repository topology.
...
Every GitHub pull-request and issue query names its repository, because the GitHub CLI otherwise infers one from the checkout's remotes and prefers `upstream`, which makes an unpinned query inside a fork answer for the parent project without saying so.
[`bin/fm-gh-lib.sh`](../bin/fm-gh-lib.sh) is the resolution owner: it takes the repository from a canonical PR URL, an explicit `--repo`, or the `origin` remote, and refuses the query when none of those applies, so a caller gets no answer rather than a wrong one.
