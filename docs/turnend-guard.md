...
The guard treats that stood-down outcome as its own first-class case: it accounts and bounds it on a separate counter, `state/.turnend-standdown-blocks`, incrementing once per turn rather than deduping against an epoch that cannot advance, and its block message names the real cause instead of directing the operator to inspect hook registration.
Once that counter exceeds the same `FM_CLAUDE_TURNEND_BLOCK_BUDGET`, it re-checks watcher health and the foreign-owner condition fresh and, if still stood down, fires the same one-time attended fail-open.
It shares `state/.claude-autoarm-failure-alarmed` with the ordinary progression, so only one attended alarm ever fires per unresolved episode regardless of which reason triggered it first, and positive watcher recovery clears both counters together.
...
`tests/fm-turnend-guard.test.sh` covers the predicate, main and secondmate primary scope, child-worktree exclusion, `FM_HOME` and `FM_STATE_OVERRIDE` precedence, the live-lock and fresh-beacon guard predicate, the cooperative `--claude` claim wait, monotonic failed-epoch progression, bounded attended fail-open, post-alarm continuation suppression, positive recovery reset, the stood-down-inert path (bounded progression, its own reason message, the shared one-time alarm, and that a dead or absent lock owner keeps the ordinary path), Pi logical-run latching, missing-`jq` behavior, all five primary registrations, Grok native and legacy selection, typed field precedence, malformed input, and exactly-one-path safety.
