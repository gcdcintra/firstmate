...
Both guards evaluate the same live lock, process identity, home/path binding, and fresh-beacon predicate, and they deliberately act on it differently.
The turn-end guard blocks whenever that strict predicate fails, because blocking is what gets a watcher armed.
The pull-based guard alarms only once the beacon has also aged past `FM_GUARD_GRACE`, because a watcher releases its lock every time it exits on a wake and the strict predicate is false throughout that ordinary between-cycle gap; `bin/fm-guard.sh`'s header owns that argument in full.
...
`bin/fm-guard.sh` calls the same predicate, but uses it to name the failing condition in its banner rather than to decide whether to alarm; its alarm gate is `fm_supervision_unhealthy` from `bin/fm-supervision-lib.sh`.
On failure the predicate sets `FM_WATCHER_UNHEALTHY_REASON` and `FM_WATCHER_UNHEALTHY_DETAIL`, rendered by `fm_watcher_unhealthy_description` beside it, so both banners report the condition that actually failed and an unrecognized future token still renders as itself.
...
When `state/.lock` instead records a live harness process this session cannot match to its own, the Stop-owned auto-arm stays inert (`fm_session_lock_foreign_owner_alive` in `bin/fm-session-lock-lib.sh`) and never writes an epoch or an exhausted-failure notice, so the ordinary progression above can never become reachable while waiting on that notice (2026-08-08 incident: 28 hours of continuous blocking with `state/.turnend-claude-blocks` stuck at `count=1`).
A live owner this session proves is its own quiescent fork source is not foreign, so it never reaches this stood-down path at all ([Session fork recovery](watcher-continuity.md#session-fork-recovery)).
An owner whose fork continuity simply could not be proven does reach it, so the banner and the alarm name the recorded pid the deciding read actually saw, and report which reading its evidence supports - a second live session, or this session's own displaced predecessor - instead of asserting either or asking the operator to establish it.
No command available to the operator separates those readings, because `bin/fm-lock.sh status` prints the same live-pid line for both, so the refusing sub-check is published by the shared predicate and rendered once for both the stderr banner and the JSON alarm.
An owner that resolves to a session whose transcript has diverged reads as a second live session in use, and an owner that is a live harness of another kind reads as a primary of another kind holding the home, which rules the displaced predecessor out.
An owner that is a live Claude process no session record vouches for is reported as inconclusive and names both readings it is equally consistent with, because the evidence genuinely does not pick one and claiming otherwise is the overstatement this half of the change exists to remove.
The guard treats that stood-down outcome as its own first-class case: it accounts it on the same `state/.turnend-claude-blocks` budget under the same budget lock, incrementing once per turn rather than deduping against an epoch that cannot advance, and passing that record's `epoch` field through unchanged so a later return to the ordinary progression still reads the epoch that progression recorded.
Its block message names the recorded owner and keeps the hook registration in scope for inspection, because a broken hook is silent in exactly the same way (2026-08-14 incident: an evening of blocks whose message ruled that inspection out).
One shared counter is deliberate: two independent counters would let the two re-block reasons each reach `FM_CLAUDE_TURNEND_BLOCK_BUDGET` in turn and stack to twice it, past the margin the budget keeps below Claude's 8-block override.
...
`tests/fm-guard-stale-banner.test.sh` covers the matching pull-guard predicate in both directions: the between-cycle gap (fresh beacon, absent lock, watcher process alive) stays silent, while a stale beacon with an absent lock, a lock naming a dead pid, and a live but non-beating lock holder each alarm and name that exact condition.
