...
No command available to the operator separates those readings, because `bin/fm-lock.sh status` prints the same live-pid line for both, so the refusing sub-check is published by the shared predicate and rendered once for both the stderr banner and the JSON alarm: a recorded owner no live session record vouches for reads as the displaced predecessor, while one that resolves to a session whose transcript has diverged reads as a second live session in use.
