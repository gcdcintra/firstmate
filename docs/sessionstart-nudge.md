...
| --- | --- | --- |
| Claude | `.claude/settings.json` registers `SessionStart` for `startup`, `resume`, and `clear`, excludes `compact` and `fork`, and invokes the wrapper through `CLAUDE_PROJECT_DIR`. | Native stdout context injection is supported. |
...
`fork` is excluded deliberately.
A forked continuation carries the conversation it was forked from, so it needs no second session-start digest, and injecting one would re-run the mutating startup sweeps mid-conversation.
It re-claims the home through the Stop-owned auto-arm instead ([`watcher-continuity.md`](watcher-continuity.md#session-fork-recovery)).
