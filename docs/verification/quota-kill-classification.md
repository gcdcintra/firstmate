...
- **A new vendor wording.** The recognizer matches the two sentences above.
  A reworded limit message is not recognized, which reports as a plain failure - today's behavior - never as an excused one.
- **A quotation of the vendor sentence as an agent's last words.** Adjacency narrows the quotation false positive; it does not eliminate it.
  A genuine agent failure whose FINAL output line quotes the vendor sentence is still reported as a usage-limit kill, because the harness's own real exit line follows that line and satisfies the adjacency requirement.
  The quotation does not have to include an exit line of its own - the real one supplies the adjacency - and an intervening blank line does not change that.
  The session family qualifies on the sentence alone, since it carries no action-link second anchor.
  What adjacency does exclude is a quotation followed by any further agent output before the harness exits, which is the shape `tests/fm-quota-kill.test.sh` pins.
  The evidence field still carries whatever the quoting agent wrote after the sentence, so the leaked trailing prose is visible to anyone reading the detail.
  Scanning this whole record in log mode happens not to match, but only incidentally - its later prose mentions the `started pid=` marker, which trips the final-episode rule - so that is a coincidence of this file, not a property to rely on.
- **Any vendor sentence at all, in pane mode.** Plain mode carries no adjacency requirement by design, so tracked text quoting either wording still matches it, including this record and `tests/fm-quota-kill.test.sh`.
  The blocked-pane consumer is enrichment only: a false match renames a wake that was already being raised and never suppresses one, which is why this exposure is accepted rather than closed by narrowing the mode that acceptance criterion 2 depends on.
- **A kill before any output.** An agent killed before writing its first line leaves no message to match.
- **Non-`claude` harnesses.** Only the `claude` wordings are established.
  `codex`, `opencode`, `pi`, `grok` and `kimi` limit messages have not been observed here.
- **The pane dialog's own wording.** The blocked-pane enrichment matches the same two vendor sentences.
  If the interactive dialog words its limit differently, the enrichment does not fire and the wake keeps its existing generic reason - it is never suppressed.
- **A stopped supervisor.** The immediate blocked escalation needs a live supervision cycle.
  When none is running, the guard and auto-arm path owns that, not this.
- **Backends with no native push.** The prompt blocked wake rides the Herdr `pane.agent_status_changed` stream (`docs/herdr-backend.md`).
  A tmux home keeps the poll loop's wedge timer.
