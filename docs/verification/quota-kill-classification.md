...
- **A quotation of the vendor sentence as an agent's last words.** Adjacency narrows the quotation false positive; it does not eliminate it. A genuine agent failure whose FINAL output line quotes the vendor sentence is still reported as a usage-limit kill, because the harness's own real exit line follows that line and satisfies the adjacency requirement. The quotation does not have to include an exit line of its own - the real one supplies the adjacency - and an intervening blank line does not change that. The session family qualifies on the sentence alone, since it carries no action-link second anchor. What adjacency does exclude is a quotation followed by any further agent output before the harness exits, which is the shape `tests/fm-quota-kill.test.sh` pins. The evidence field still carries whatever the quoting agent wrote after the sentence, so the leaked trailing prose is visible to anyone reading the detail. Scanning this whole record in log mode happens not to match, but only incidentally - its later prose mentions the `started pid=` marker, which trips the final-episode rule - so that is a coincidence of this file, not a property to rely on.
