...
Attribution is not re-derived for this mode: it reuses whatever binding the rest of [`bin/fm-crew-state.sh`](../../bin/fm-crew-state.sh) already applies, which since 2026-08-19 leads with the CLI's own run-id binding and keeps the commit-sha tests as its fallback ([`docs/verification/crew-state-attribution.md`](crew-state-attribution.md) holds the evidence for both).
