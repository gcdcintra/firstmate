...
gh_base_repo_ambiguous() {  # <dir>
  ...
  while read -r name url _; do
    ...
    case "$url" in
      https://github.com/*|ssh://git@github.com/*|git@github.com:*) return 0 ;;
    esac
...
# when already pinned or when nothing is ambiguous; see this script's header for
# the full contract.
gh_base_repo_pin() {
  local origin_repo
  git -C "$FM_ROOT" rev-parse --git-dir >/dev/null 2>&1 || return 0
  # An existing gh-resolved setting means the choice is already made explicitly.
  if git -C "$FM_ROOT" config --get-regexp '^remote\..*\.gh-resolved$' >/dev/null 2>&1; then
    return 0
  fi
