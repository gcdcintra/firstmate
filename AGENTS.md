...
state/               volatile runtime signals; gitignored
  ...
  .claude-autoarm.lock .claude-autoarm-epoch .claude-autoarm-failure-notified .claude-autoarm-failure-alarmed .turnend-claude-blocks .turnend-claude-blocks.lock   Claude Stop auto-arm single-flight, epoch, failure-episode, attended-alarm, guard-budget, and budget-lock records; the one guard-budget record bounds both re-block reasons (auto-arm failure and standing down because the fleet lock records a live owner this session cannot match to its own); never touch
  .hash-* .count-* .stale-* .stale-since-* .paused-* .wedge-* .cpu-* .gone-* .seen-* .hb-surfaced-* .last-* .heartbeat-streak   watcher internals; never touch
...
2. **Bootstrap** - detect-only checks (tool/version problems, GitHub auth, the worktree-tangle check, harness override, dispatch-profile validation, backlog-backend status) always run, but routine confirmations stay silent by default.
   ...
   Home-local stale Herdr projection cleanup and the six bootstrap MUTATING sweeps - non-executing legacy PR-check migration, fleet sync, the local secondmate fast-forward sweep, the secondmate liveness sweep, X-mode artifact writes, and the local GitHub base-repository pin that keeps pull-request, issue, and startup-banner figures answering for this repo - run only when this session actually holds the lock from step 1.
...
Never ask about a pull request or issue without naming its repository, with `--repo <owner>/<name>` or a full PR URL: the GitHub CLI infers a repository from the checkout's remotes and prefers `upstream`, so inside a fork an unpinned query answers about the parent project and reads exactly like a correct answer.
Firstmate's own scripts get this from `bin/fm-gh-lib.sh`, which refuses rather than guessing, and `bin/fm-gh-repo-guard.sh` keeps every `bin/` caller pinned.
...
Read `bin/fm-quota-advice.sh` first, because a validation run whose agent is killed by the account usage limit can strand the branch, and the account, weekly, and per-model windows bound it independently.
It reports quota and never withholds a spawn; firstmate still owns the dispatch decision and never answers a captain's wave size with a smaller fleet.
...
- teardown -> cleanup.
- wake, watcher, heartbeat, stale, gone, signal, or check -> notification, monitoring, waiting too long, stopped responding, or the worker was killed.
...
- `bootstrap-diagnostics` - load whenever the session-start digest's bootstrap section prints an actionable diagnostic line (`MISSING:`, `MISSING_MANUAL:`, `BACKEND_INVALID:`, `NEEDS_GH_AUTH`, `TANGLE:`, `STARTUP_MEMORY_BUDGET:`, `CREW_DISPATCH: invalid`, `FLEET_SYNC:`, `PR_CHECK_MIGRATION:`, `SECONDMATE_SYNC:`, `SECONDMATE_LIVENESS:`, `NUDGE_SECONDMATES:`, `GH_BASE_REPO:`, or `FMX:`); silence and `BOOTSTRAP_INFO:` need no load.
...
- `project-management` - load before adding, creating, removing, or initializing a project.
  Cloning or registering a project is add intake and uses the same trigger.
- `stuck-crewmate-recovery` - load when the session-start digest reports an ordinary direct report's endpoint dead or its metadata has no window, or after a stale or gone wake, looping pane, repeated confusion, an answered-by-brief question, an unresponsive crewmate, or a failed steer.
