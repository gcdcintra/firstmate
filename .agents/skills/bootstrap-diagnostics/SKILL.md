...
- `GH_BASE_REPO: <remediation>` - GitHub pull-request and issue queries in this checkout answer for a different repository than its own origin, because the GitHub CLI infers a base repository from the remotes and prefers `upstream` while no `remote.<name>.gh-resolved` setting exists.
  ...
  A locked session pins the base repository itself and reports `BOOTSTRAP_INFO` instead; this line means the session was read-only, or the pin failed.
  Run the printed `gh repo set-default` command when the session holds the fleet lock, and until then name the repository on every pull-request and issue query, as `AGENTS.md` section 3 requires.
