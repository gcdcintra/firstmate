...
- `claude`: verified native `SessionStart` stdout injection; `.claude/settings.json` matches `startup`, `resume`, and `clear`, but not `compact` or `fork`.
...
OpenCode and Claude Code can accept and queue an Enter while leaving text visible, Grok can consume Enter in its slash popup without submitting, and Kimi can silently drop a message sent before readiness even though the send returns success.
...
| Fact | Value |
|---|---|
| Busy state | Owned lifecycle hooks: `UserPromptSubmit` opens a turn, `Stop`, `StopFailure`, and `SessionEnd` close it. Claude fires no hook for a manual interrupt, so a firstmate-initiated interrupt must record every hook-owned clear itself (`bin/fm-send.sh`). |
| Exit command | `/exit` |
...
This is separate from the per-task crewmate hooks above (`bin/fm-spawn.sh` writes those into a task's own `.claude/settings.local.json`).
...
**Busy-queued Enter (opencode 1.18.4, tmux and herdr backend fixes).**
...
re-send), while an idle pane keeps `pending` as a genuine swallow. This
composer-proof form is tmux-specific; `docs/herdr-backend.md` owns the herdr
adapter's own native-state form of the conversion and its residual limit.
