...
- **A watched default branch** - after work merges, the watcher keeps reading each cloned project's default branch and speaks up once when it settles red, naming the suspect merge and the last green commit; the alert carries the failing job's step count and duration, so a check that ran and failed is never confused with one that never started. Turn it off per home with a `config/branch-watch` file containing `off`.
